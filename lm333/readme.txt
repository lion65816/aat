...
______________________________________________________________________
 ...
 worked on and improved at various times over the past 22 years.
...
Version 3.33 October 1, 2022

-fixed a bug from 3.32 where the music dialog in the level editor was
 setting the music bypass value to value-1 after hitting ok.  Thanks
 goes out to Imamelia for reporting this.
-made it possible to use the "8x8 Select" button in the ExAnimated
 dialogs to also enable using the palette editor for filling in
 colors for the frames to use and the destination using
 Ctrl+Left-Click.
-made it possible to use Ctrl+Shift+Left-Click in the Palette Editor
 on ExAnimated color destinations to select their slot in the
 ExAnimated dialog window (it will open the ExAnimated dialog if there
 isn't another dialog window already open).
-moved the Ctrl+Alt+Left-Click keyboard shortcut in the 8x8
 editor/selector to Ctrl+Shift+Left-Click, to make it consistent with
 the palette editor.


Version 3.32 September 24, 2022 (22 year Anniversary of Lunar Magic!)

-fixed a bug from 1.00 where if there was only a single object on
 screen 0 that extended into screen 1, you could often not move an
 object on screen 1 behind it with the "Send Backward" or "Decrease Z
 Order" commands, although "Send to Back" would work.  Thanks goes out
 to MarioFanGamer for reporting this.
-fixed a bug from 2.50 with the layer 3 16x16 tile unofficial display
 mode in the level editor where it wouldn't draw some tiles.  Thanks
 goes out to DPBOX for reporting this.
-changed what the "Unusable Space" value shows in the "Scan ROM"
 command to match what the help file says, as apparently it was
 calculating something else from a long time ago that hasn't been
 relevant since 1.63.
-changed some of the entrance/exit icons a bit on the toolbar.  Thanks
 goes out to Burning Loaf for providing these.
-made it so that the sprite size table is also loaded right before
 level save, not just on level load.
-added a new "Remap" button to the ExAnimated dialogs for remapping
 frames and destinations.
-added a new "8x8 Select" button to the ExAnimated dialogs to enable a
 point-and-click mode using the 8x8 editor/selector for filling in
 tile numbers for the frames to use and the destination.  It will
 automatically advance to the next frame field after each click, but
 you can also manually click on a field to change which one is being
 done.  The button will also open the 8x8 editor/selector if it isn't
 already open, and auto-enable viewing the animated tile area.
-made it possible to double click on frame and destination values in
 the ExAnimated dialogs to show the tile in the 8x8 editor/selector
 when in "8x8 Select" mode.
-made it possible to use Ctrl+Alt+Left-Click in the 8x8
 editor/selector on ExAnimated tile destinations to select their slot
 in the ExAnimated dialog window (it will open the ExAnimated dialog
 if there isn't another dialog window already open).  You can also
 just hold down the keyboard shortcut keys while moving the mouse
 around to see in the status bar which slot last wrote to that tile.
-added an option to save Luigi's map position in saved games instead
 of saving a copy of Mario's in the overworld's "Extra Options"
 dialog.
-the program will now offer to clear a recent file when you attempt
 opening one that doesn't exist.
-added a "Clear Recent Files" menu item to the recent files list.
-made it possible to type music track numbers in the drop down list
 box for bypassing music in levels.
-added ExpandROM to command line functions.
