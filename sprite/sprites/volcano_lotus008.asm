...
;;Ruby note: Wow, imagine not commenting your code.
...
print "INIT ",pc
        ...
        AND #$04
        BEQ +
        TYA                     ;Why not just load #$80 here?
+       STA !1FD6,x             ;If extra bit is set, negative, otherwise zero.
        RTL
...
spawn_pollen:
        LDA !15A0,x 
        ORA !186C,x 
        BNE .return                     ;If offscreen, don't spawn pollen, I guess.
        LDA.b #!pollen_to_spawn-1
        STA $00
.pollen_loop
        LDY #$07                        ;Fill empty slots from top:
.find_slot_loop
        LDA $170B|!Base2,y
        BEQ .spawn_pollen
        DEY
        BPL .find_slot_loop
        RTS                             ;Out of slots, return

.spawn_pollen
        LDA #!pollen_number
        STA $170B|!Base2,y              ;Designate slot as pollen
        LDA !E4,x                       ;Get lotus X position
        CLC
        ADC #$04
        STA $171F|!Base2,y              ;Pollen X position
        LDA !14E0,x                     ;Repeat for high byte
        ADC #$00
        STA $1733|!Base2,y
        LDA !D8,x                       ;Y low byte
        STA $1715|!Base2,y
        LDA !14D4,x                     ;Y high byte
        STA $1729|!Base2,y
        LDX $00                         ;Set pollen speeds
...
;         BPL .no_home
;         LDA #$01
;         STA $1765|!Base2,y            ;Ah, here's the bug
; .no_home                              ;Code only *sets* the byte, never clears it.
        ;Fix:
        LDA #$00
        BIT !1FD6,x
        BPL .homeless
...
.homeless
        ...
        BMI .return
        EOR #$01                        ;why
        STA !1FD6,x                     ;returns the value in 1fd6, I guess
.return                                 ;but it gets immediately overwritten...
        RTS
